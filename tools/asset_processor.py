#!/usr/bin/env python3
"""
Character Sheet Asset Processor
Automatically removes backgrounds, extracts sprites, and organizes game assets
"""
...
class AssetProcessor:
    ...
    def load_config(self, config_file: str) -> Dict:
        ...
        default_config = {
            ...
            "background_removal": {
                "method": "rembg",  # "rembg" or "color_key"
                "color_key": [255, 255, 255],  # RGB values for color key
                "tolerance": 30
            },
            "output_format": "PNG",
    ...
    def detect_grid(self, image: Image.Image) -> Tuple[int, int]:
        ...
        rows = len(h_lines) + 1 if h_lines is not None else self.config["grid_detection"]["manual_rows"]
        cols = len(v_lines) + 1 if v_lines is not None else self.config["grid_detection"]["manual_cols"]
    ...
    def process_character_sheet(self, image_path: str, character_name: str = None) -> Dict[str, str]:
        ...
        if self.config["background_removal"]["method"] == "rembg":
            processed_image = self.remove_background_rembg(image)
        else:
...
def main():
    ...
    processor = AssetProcessor(args.config)
